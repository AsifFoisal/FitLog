...
const Stats = () => {
    ...
    const { addWorkout, saveWorkout, activeTab,isLoaded } = context;
        ...
        <div>
            <div className="grid grid-cols-3 divide-x divide-white/10 rounded-2xl bg-[#101218] p-6 shadow-lg my-6">
                <div className="flex flex-col gap-2 pl-2">
                    ...
                    <span className="font-oswald text-4xl font-bold text-[#CCFF00]">{isLoaded ? totalExercises : 0}</span>
                </div>


                <div className="flex flex-col gap-2 pl-8">
                    <span className="text-xs font-medium text-zinc-400">Minutes</span>
                    <span className="font-oswald text-4xl font-bold text-white">{isLoaded ? totalMinutes : 0}</span>
                </div>


                <div className="flex flex-col gap-2 pl-8">
                    <span className="text-xs font-medium text-zinc-400">Calories</span>
                    <span className="font-oswald text-4xl font-bold text-white">{isLoaded ? totalCalories : 0}</span>
                </div>
            </div>
        </div>
    );
